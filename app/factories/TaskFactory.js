...
app.factory("TaskFactory", function($q, $http, fbcreds, $route){
    ...
    const getUsersTasks = (userId) => {
        return $http.get(`${fbcreds.databaseURL}/user/${userId}/tasks.json`)
        .then(res => res.data)
        .catch(console.error);
    };

	return { getAllTasks, createTask, getTask, updateTask, toggleTaskComplete, deleteTask, getUsersTasks };
